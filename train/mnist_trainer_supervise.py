...
class MnistTrainer(ConstractiveTrainer):
    ...
    def setup_loss_weights(self):
        ...
        self.mnist_weight = 1.0
        self.mnist_auxiliary_weight = 1.0
    ...
    def train(self):
        ...
        for self.curr_epoch in progress_bar:
            ...
            progress_bar.set_postfix({
                ...
                "Mni.": f"{epoch_metrics['mnist']:.4f}",
                "Mna.": f"{epoch_metrics['mnist_auxiliary']:.4f}",
    ...
    def compute_losses(self, data, embedding_features, auxiliary_features, latent_features, reconstructed_data, labels):
        return {
            ...
            "mnist": self.mnist_loss(data, embedding_features, labels),
            "mnist_auxiliary": self.mnist_auxiliary_loss(data, auxiliary_features, labels),
    ...
    def total_loss(self, data, embedding_features, auxiliary_features, latent_features, reconstructed_data, labels):
        return (
            ...
            self.mnist_weight * self.mnist_loss(data, embedding_features, labels) +
            self.mnist_auxiliary_weight * self.mnist_auxiliary_loss(data, auxiliary_features, labels) +
    ...
    def mnist_loss(self, data, embedding_features, labels):
    ...
    def mnist_auxiliary_loss(self, data, auxiliary_feature, labels):
        data = data.view(data.size(0), -1)
        pos_indices = self.get_positive_indices_batch(
            data, labels, k=self.k)  # 默认 labels=None
        embedding_sim_features = self.get_embeddings_for_loss(
