...
class ConvBlock(nn.Module):
    def __init__(
        ...
        stride: int,
        padding: int,
        conv_dim: int = 2,  # Changed from 'dim' to 'conv_dim'
        relu: bool = True,
    ):
        super().__init__()

        # Choose convolution and batch normalization layers based on dimension
...
class ResBlock(nn.Module):
    def __init__(
        ...
        # Shortcut connection if input and output channels are different
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        x = self.conv2(x)
        x = x + identity  # Add the shortcut connection
        x = nn.ReLU()(x)  # Apply ReLU after addition
        return x
